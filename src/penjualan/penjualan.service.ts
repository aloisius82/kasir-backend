...
export class PenjualanService {
    ...
    async create(data: any, user: any) {
        ...
        const Query = {
            data: {
                ...
                paymentRef: data.paymentRef ? data.paymentRef : null,
                detailPenjualan: {
        ...
        dbRes.detailPenjualan.forEach((detail) => {
            this.prisma.barang
                ...
                .then((barang) => {
                    if (barang) {
                        if (!barang.hpp.equals(new Decimal(0)))
    ...
    async getMonthlyItemReport(month?: string) {
        let formattedMonth: string
        if (month && /^\d{4}-\d{2}$/.test(month)) {
            formattedMonth = month
        } else {
            // Default ke bulan saat ini dalam UTC+7 (WIB)
            const now = new Date()
            // Menambahkan 7 jam ke waktu UTC untuk mendapatkan waktu lokal Indonesia
            const utc7Date = new Date(now.getTime() + 7 * 60 * 60 * 1000)
            formattedMonth = utc7Date.toISOString().slice(0, 7)
        }

        return this.prisma.$queryRaw`
            SELECT
            DATE_FORMAT(CONVERT_TZ(p.tanggal, '+00:00', '+07:00'), '%Y-%m') AS bulan,
            dp.barangId,
            b.nama,
            dp.hargaSatuan,
            SUM(dp.jumlah) AS jumlah,
        ...
        JOIN Penjualan p ON dp.penjualanId = p.id
        JOIN Barang b ON dp.barangId = b.id
        WHERE DATE_FORMAT(CONVERT_TZ(p.tanggal, '+00:00', '+07:00'), '%Y-%m') = ${formattedMonth}
        GROUP BY
            DATE_FORMAT(CONVERT_TZ(p.tanggal, '+00:00', '+07:00'), '%Y-%m'),
            dp.barangId,
            b.nama,
            dp.hargaSatuan
        ORDER BY b.nama ASC;`
    ...
    async getPenjualanBulananPerBarang(bulan: string) {
        ...
        const startDate = new Date(
            `${year}-${month.toString().padStart(2, '0')}-01T00:00:00.000+07:00`
        )
